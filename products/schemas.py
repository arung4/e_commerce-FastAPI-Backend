...
class ProductBase(BaseModel): 
    ...
    def validate_name(cls,v):
        ...
        if not re.match(r'^[A-Za-z0-9\s\-@%]+$',v):
            raise ValueError("Name can only contain letters, numbers , spaces and basic punctuations.")
    ...
    def validate_description(cls, v):
        ...
        if not re.match(r'^[A-Za-z0-9\s.,;:!?\'"-]+$', v):
            raise ValueError("Description can contain only letters, numbers, spaces, and basic punctuation")
        if len(v) > 200:
