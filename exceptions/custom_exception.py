...
class ProductNotFoundException(HTTPException):
    def __init__(self, detail: str = "Product not found with this ID"):
        super().__init__(status_code=404, detail=detail)

    
class ProductAlreadyExistsException(HTTPException):

    def __init__(self,detail: str = "Product already thier with this name"):
        super().__init__(status_code=status.HTTP_400_BAD_REQUEST, detail=detail)
...
class ProductStockUnavailableException(HTTPException):
     def __init__(self, detail : str = "Not enough stock available "):
        super().__init__(status_code = status.HTTP_400_BAD_REQUEST, detail=detail)
